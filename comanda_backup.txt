...
export default function Comanda() {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .update({ 
          ...
          arquivo_url: `${contaId}/${nomeArquivo}`,
  ...
  const calcularSubtotal = (id: number | null) => {
    ...
    if (!comanda) return 0;
    return (comanda.itens || []).reduce((sum: number, item: any) => {
      return sum + item.quantidade * item.preco;
    }, 0);
  };

  const adicionarItem = (id: number | null, item: any) => {
    if (!id) return;
    const comanda = comandas.find((c) => c.id === id);
    if (!comanda) return;
    ...
    const novoItem = {
      ...
      preco: parseFloat(item.preco || precoRapido),
      quantidade: 1,
    };

    const novasComandas = comandas.map((c) =>
      c.id === id ? { ...c, itens: [...(c.itens || []), novoItem] } : c
    );
  ...
  const removerItem = (id: number | null, itemId: number) => {
    if (!id) return;
    const novasComandas = comandas.map((c) =>
      c.id === id ? { ...c, itens: (c.itens || []).filter((i: any) => i.id !== itemId) } : c
    );
    setComandas(novasComandas);
  };

  const deletarComanda = async (id: number | null) => {
    if (!id) return;
            ...
            <div key={comanda.id} className="bg-white p-4 rounded-lg shadow-md border-l-4 border-blue-600">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-gray-600">{(comanda.itens || []).length} itens</p>
                </div>
        ...
        {modalAberto !== null && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 max-w-md w-full max-h-screen overflow-y-auto">
              ...
              <div className="mb-4">
                <h3 className="font-bold mb-2">Itens na Comanda</h3>
